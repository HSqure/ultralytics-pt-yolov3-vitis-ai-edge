...
class Detect(nn.Module):
    ...
    #         if cnt==0:
    #             yv=torch.full((1,nx), item, dtype=int)
    #             xv=x.view(1,nx)
    #         else:
    #             yv=torch.cat((yv, torch.full((1,nx), item, dtype=int)), 0)
    ...
    #     for cnt, item in enumerate(y):
    #         if cnt==0:
    #             yv=np.full((1,nx),item)
    #             xv=x.reshape(1,nx)
    #         else:
    #             yv=np.concatenate((yv, torch.full((1,nx),item)), 0)
    #             xv=np.concatenate((xv, x.reshape(1,nx)), 0)

    #     return torch.tensor(np.stack((xv, yv), 2)).float() #.resize((1, 1, ny, nx, 2)))
