  ...
  Mat frame;
	...
	while (true) {
		...
		if (queueShow.empty()) {
            ...
            cv::imshow("ADAS Detection@ALINX", frame);
 ...
 * @param frame input picture frame
 * @param sWidth image width
 * @param sHeight image height
 *
 * @return none
 */
void postProcess(vart::Runner* runner, Mat& frame, vector<float*> results,
                 int sWidth, int sHeight) {
    const string classes[1] = {"person"};
    ...
    for (size_t i = 0; i < res.size(); ++i) {
            float xmin = (res[i][0] - res[i][2]) * w + 1.0;
            float ymin = (res[i][1] - res[i][3]) * h + 1.0;
            float xmax = (res[i][0] + res[i][2]) * w + 1.0;
            float ymax = (res[i][1] + res[i][3]) * h + 1.0;
...
int main(const int argc, const char** argv) {
    ...
    if (argc != 3) {
		cout << "Usage of pedestrian detection: " << argv[0] << " <video_file> <model_file>"
				<< endl;
		return -1;
    }
