  ...
  while (true) {
    ...
    if (queueShow.empty()) {
      mtxQueueShow.unlock();
		...
		std::cout << "Input Image:" << "./data/" + images[idxShowImage] << std::endl;
		std::cout << "Output Image:" << "./output/" + images[idxShowImage] << std::endl;
...
int main(const int argc, const char** argv) {
	...
	if (argc != 2) {
	cout << "Usage of ADAS detection: ./adas_detection "
			"[model_file] "
			<< endl;
	return -1;
	}
