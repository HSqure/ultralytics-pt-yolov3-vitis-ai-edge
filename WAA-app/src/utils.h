...
void correct_region_boxes(vector<vector<float>>& boxes, int n,
    ...
    for (int i = 0; i < n; ++i){
        ...
        boxes[i][2] *= (float)new_w/netw;
        boxes[i][3] *= (float)new_h/neth;
    }
}
 ...
 *
 * @param boxes new boxes to detect
 * @param channel output layer channel number ( 4+1+n: coordinate(4), confidence(1), category(n) <-one hot encode)
 * @param height Net output layer height
 * @param width Net output layer width
 * @param num output layer number (one of 3)
 * @param sWidth Net input data width
 * @param sHeight Net input data height
 *
 * @return none
 */
void detect(vector<vector<float>> &boxes, vector<float> result,
    int channel, int height, int width, int num, int sHeight, int sWidth) {
    vector<float> biases{ 21,99, 28,146, 33,154,   
                          7,39, 10,56, 10,52,  
                          2,10, 3,15, 4,20 };
    int conf_box = 5 + classificationCnt; // object infomation part:  coordinate(4), confidence(1), category(n) <-one hot encode
    ...
    for (int h = 0; h < height; ++h) {
        for (int w = 0; w < width; ++w) {
            for (int c = 0; c < anchorCnt; ++c) {
                ...
                // std::cout << "Output layer: " << num << std::endl;
                // std::cout << "box shape: " << (exp(swap[h * width + w][c][2]) * biases[2 * c + 6 * num]) << " x "
                //             << (exp(swap[h * width + w][c][3]) * biases[2 * c + 6 * num + 1])
                //             << std::endl;
                std::cout << "Conf score: " << obj_score << std::endl;
                // std::cout << "Net input data Shape: " << sWidth << " x " << sHeight << std::endl;
                // std::cout << "Frame Shape: " << width << " x " << height << std::endl;
                                
                box.push_back((w + sigmoid(swap[h * width + w][c][0])) / width);
                box.push_back((h + sigmoid(swap[h * width + w][c][1])) / height);
                box.push_back(exp(swap[h * width + w][c][2]) * biases[2 * c + 6 * num] / float(sWidth));
                box.push_back(exp(swap[h * width + w][c][3]) * biases[2 * c + 6 * num + 1] / float(sHeight));
                box.push_back(-1);
                box.push_back(obj_score);
                // class record
                for (int p = 0; p < classificationCnt; p++) {
                    box.push_back(obj_score * sigmoid(swap[h * width + w][c][5 + p]));
                    std::cout << "One hot class: " << sigmoid(swap[h * width + w][c][5 + p]) << std::endl;
                }
                boxes.push_back(box);
            }
        }
...
void get_output(float* dpuOut, int sizeOut,  int oc, int oh, int ow, vector<float>& result) {
    ...
    for(int a = 0; a < oc; ++a){
        for(int b = 0; b < oh; ++b){
            for(int c = 0; c < ow; ++c) {
                ...
                result[a * oh * ow + b * ow + c] = dpuOut[offset];
            }
        }
    }
}
